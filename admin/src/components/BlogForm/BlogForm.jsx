...
const BlogForm = () => {
  ...
  const blog = useSelector((state) =>
    state.blogs.find((blog) => blog.blog_id === Number(id))
  );
  const [blogTitle, setBlogTitle] = useState(blog?.blog_title || "");
  const [editorState, setEditorState] = useState(
    id
      ? EditorState.createWithContent(
          convertFromRaw(JSON.parse(blog.blog_body))
        )
  ...
  const publishBlog = () => {
    ...
    if (id) {
      dispatch(
        updateBlog(id, { blogTitle, blogBody, blogSlug, isPublished: 1 })
      );
    } else {
      dispatch(
        addBlog({
  ...
  const saveDraft = () => {
    ...
    if (id) {
      dispatch(
        updateBlog(id, { blogTitle, blogBody, blogSlug, isPublished: false })
      );
    } else {
      dispatch(
        addBlog({
  ...
  const onEditorStateChange = (richTextEditorState) =>
    setEditorState(richTextEditorState);
        ...
        <Paper className={classes.root}>
          ...
          <TextField
            ...
            onChange={(e) => {
              ...
              setBlogSlug(
                `/${e.target.value.toLowerCase().trim().replace(/ /g, "-")}`
              );
            }}
          />
          <Typography variant="h5" className={classes.subTitle}>
